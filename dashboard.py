...
@st.cache_resource(show_spinner="Memuat dataset dan melatih model Decision Tree (TF-IDF)...")
def load_and_train_model():
    df = pd.read_csv('train.csv')
    
    # Clean text
    # Menggunakan df_target['text'] karena di script Anda sebelumnya mengambil dari kolom text
    ...
    # Model Decision Tree Eks-3
    dt_tfidf = DecisionTreeClassifier(random_state=42, max_depth=5)
    dt_tfidf.fit(X_tfidf, y)
    
    return tfidf, dt_tfidf

# Panggil fungsi training (Hanya dipanggil sekali saat aplikasi dijalankan)
tfidf_vectorizer, dt_model = load_and_train_model()

# --- TAMPILAN DASHBOARD ---
st.title("🌾 Prediksi Kategori Artikel Pertanian (NLP)")
st.write("""
Dashboard ini menggunakan model **Decision Tree** dengan ekstraksi fitur **TF-IDF** (Eksperimen 3) 
untuk memprediksi klasifikasi multi-label artikel ke dalam 41 kategori pertanian.
""")
    ...
    if st.button("Analisis Teks", type="primary"):
        if user_input.strip():
            ...
            if predicted_labels:
                ...
                # Tampilkan sebagai tags
                cols = st.columns(min(len(predicted_labels), 5))
                for idx, label in enumerate(predicted_labels):
                    with cols[idx % 5]:
                        st.info(f"🏷️ **{label}**")
            else:
                st.warning("Tidak ada kategori pertanian spesifik yang terdeteksi secara kuat dari teks tersebut oleh model. Cobalah dengan teks lain yang lebih relevan.")
...
st.markdown("---")
st.subheader("📊 Evaluasi & Performa Keseluruhan")
